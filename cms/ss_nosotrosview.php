...
$ss_nosotros_view = NULL; // Initialize page object first

class css_nosotros_view extends css_nosotros {
	...
	// Page object name
	var $PageObjName = 'ss_nosotros_view';
	...
	function __construct() {
		...
		if (@$_GET["id_ssnosotros"] <> "") {
			$this->RecKey["id_ssnosotros"] = $_GET["id_ssnosotros"];
			$KeyUrl .= "&amp;id_ssnosotros=" . urlencode($this->RecKey["id_ssnosotros"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_ssnosotros"] <> "") {
				...
				$this->RecKey["id_ssnosotros"] = $this->id_ssnosotros->QueryStringValue;
			} else {
				$sReturnUrl = "ss_nosotroslist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "ss_nosotroslist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "ss_nosotroslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($ss_nosotros_view)) $ss_nosotros_view = new css_nosotros_view();

// Page init
$ss_nosotros_view->Page_Init();

// Page main
$ss_nosotros_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ss_nosotros_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ss_nosotros_view = new ew_Page("ss_nosotros_view");
ss_nosotros_view.PageID = "view"; // Page ID
var EW_PAGE_ID = ss_nosotros_view.PageID; // For backward compatibility

// Form object
var fss_nosotrosview = new ew_Form("fss_nosotrosview");

// Form_CustomValidate event
fss_nosotrosview.Form_CustomValidate = 
...
fss_nosotrosview.ValidateRequired = true;
<?php } else { ?>
fss_nosotrosview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fss_nosotrosview.Lists["x_id_idioma"] = {"LinkField":"x_id_idioma","Ajax":null,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $ss_nosotros_view->ExportOptions->Render("body") ?>
<?php if (!$ss_nosotros_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($ss_nosotros_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $ss_nosotros_view->ShowPageHeader(); ?>
<?php
$ss_nosotros_view->ShowMessage();
?>
<form name="fss_nosotrosview" id="fss_nosotrosview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ss_nosotros">
<table class="ewGrid"><tr><td>
<table id="tbl_ss_nosotrosview" class="table table-bordered table-striped">
<?php if ($ss_nosotros->imagen_url->Visible) { // imagen_url ?>
	<tr id="r_imagen_url">
		<td><span id="elh_ss_nosotros_imagen_url"><?php echo $ss_nosotros->imagen_url->FldCaption() ?></span></td>
		<td<?php echo $ss_nosotros->imagen_url->CellAttributes() ?>>
<span id="el_ss_nosotros_imagen_url" class="control-group">
<span<?php echo $ss_nosotros->imagen_url->ViewAttributes() ?>>
<?php if ($ss_nosotros->imagen_url->LinkAttributes() <> "") { ?>
<?php if (!empty($ss_nosotros->imagen_url->Upload->DbValue)) { ?>
<?php echo $ss_nosotros->imagen_url->ViewValue ?>
<?php } elseif (!in_array($ss_nosotros->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($ss_nosotros->imagen_url->Upload->DbValue)) { ?>
...
<?php if ($ss_nosotros->titulo->Visible) { // titulo ?>
	<tr id="r_titulo">
		<td><span id="elh_ss_nosotros_titulo"><?php echo $ss_nosotros->titulo->FldCaption() ?></span></td>
...
<?php echo $ss_nosotros->titulo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ss_nosotros->descripcion->Visible) { // descripcion ?>
	<tr id="r_descripcion">
		<td><span id="elh_ss_nosotros_descripcion"><?php echo $ss_nosotros->descripcion->FldCaption() ?></span></td>
		<td<?php echo $ss_nosotros->descripcion->CellAttributes() ?>>
<span id="el_ss_nosotros_descripcion" class="control-group">
<span<?php echo $ss_nosotros->descripcion->ViewAttributes() ?>>
<?php echo $ss_nosotros->descripcion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($ss_nosotros->id_idioma->Visible) { // id_idioma ?>
	<tr id="r_id_idioma">
		<td><span id="elh_ss_nosotros_id_idioma"><?php echo $ss_nosotros->id_idioma->FldCaption() ?></span></td>
...
<?php echo $ss_nosotros->id_idioma->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
fss_nosotrosview.Init();
</script>
<?php
$ss_nosotros_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ss_nosotros_view->Page_Terminate();
?>
