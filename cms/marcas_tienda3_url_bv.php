...
$marcas_tienda3_url_blobview = NULL; // Initialize page object first

class cmarcas_tienda3_url_blobview extends cmarcas {
	...
	// Page object name
	var $PageObjName = 'marcas_tienda3_url_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('marcas', 'x_tienda3_url');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('tienda3_url');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('tienda3_url')));
				if (trim(strval($this->Recordset->fields('tienda3_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('tienda3_url') . "\"");
				}
...
// Create page object
if (!isset($marcas_tienda3_url_blobview)) $marcas_tienda3_url_blobview = new cmarcas_tienda3_url_blobview();

// Page init
$marcas_tienda3_url_blobview->Page_Init();

// Page main
$marcas_tienda3_url_blobview->Page_Main();
?>
<?php
$marcas_tienda3_url_blobview->Page_Terminate();
?>
