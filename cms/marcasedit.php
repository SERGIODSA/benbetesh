...
$marcas_edit = NULL; // Initialize page object first

class cmarcas_edit extends cmarcas {
	...
	function Page_Main() {
		...
		if ($this->id_marca->CurrentValue == "")
			$this->Page_Terminate("marcaslist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->logo_url);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->tienda1_url);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->tienda2_url);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->tienda3_url);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// amigable
			$this->amigable->SetDbValueDef($rsnew, $this->amigable->CurrentValue, NULL, $this->amigable->ReadOnly);

			// logo_url
			if (!($this->logo_url->ReadOnly) && !$this->logo_url->Upload->KeepFile) {
				$this->logo_url->Upload->DbValue = $rs->fields('logo_url'); // Get original value
			...
			// tienda1_url
			if (!($this->tienda1_url->ReadOnly) && !$this->tienda1_url->Upload->KeepFile) {
				$this->tienda1_url->Upload->DbValue = $rs->fields('tienda1_url'); // Get original value
			...
			// tienda2_url
			if (!($this->tienda2_url->ReadOnly) && !$this->tienda2_url->Upload->KeepFile) {
				$this->tienda2_url->Upload->DbValue = $rs->fields('tienda2_url'); // Get original value
			...
			// tienda3_url
			if (!($this->tienda3_url->ReadOnly) && !$this->tienda3_url->Upload->KeepFile) {
				$this->tienda3_url->Upload->DbValue = $rs->fields('tienda3_url'); // Get original value
			...
			// titulo1
			$this->titulo1->SetDbValueDef($rsnew, $this->titulo1->CurrentValue, NULL, $this->titulo1->ReadOnly);

			// descripcion1
			$this->descripcion1->SetDbValueDef($rsnew, $this->descripcion1->CurrentValue, NULL, $this->descripcion1->ReadOnly);

			// titulo2
			$this->titulo2->SetDbValueDef($rsnew, $this->titulo2->CurrentValue, NULL, $this->titulo2->ReadOnly);

			// descripcion2
			$this->descripcion2->SetDbValueDef($rsnew, $this->descripcion2->CurrentValue, NULL, $this->descripcion2->ReadOnly);

			// titulo3
			$this->titulo3->SetDbValueDef($rsnew, $this->titulo3->CurrentValue, NULL, $this->titulo3->ReadOnly);

			// descripcion3
			$this->descripcion3->SetDbValueDef($rsnew, $this->descripcion3->CurrentValue, NULL, $this->descripcion3->ReadOnly);

			// tiendas_pie
			$this->tiendas_pie->SetDbValueDef($rsnew, $this->tiendas_pie->CurrentValue, NULL, $this->tiendas_pie->ReadOnly);

			// marcas_pie
			$this->marcas_pie->SetDbValueDef($rsnew, $this->marcas_pie->CurrentValue, NULL, $this->marcas_pie->ReadOnly);

			// descripcion_form
			$this->descripcion_form->SetDbValueDef($rsnew, $this->descripcion_form->CurrentValue, NULL, $this->descripcion_form->ReadOnly);
			...
			// url_facebook
			$this->url_facebook->SetDbValueDef($rsnew, $this->url_facebook->CurrentValue, NULL, $this->url_facebook->ReadOnly);

			// url_twitter
			$this->url_twitter->SetDbValueDef($rsnew, $this->url_twitter->CurrentValue, NULL, $this->url_twitter->ReadOnly);

			// url_youtube
			$this->url_youtube->SetDbValueDef($rsnew, $this->url_youtube->CurrentValue, NULL, $this->url_youtube->ReadOnly);
...
// Create page object
if (!isset($marcas_edit)) $marcas_edit = new cmarcas_edit();

// Page init
$marcas_edit->Page_Init();

// Page main
$marcas_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$marcas_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var marcas_edit = new ew_Page("marcas_edit");
marcas_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = marcas_edit.PageID; // For backward compatibility

// Form object
var fmarcasedit = new ew_Form("fmarcasedit");

// Validate form
fmarcasedit.Validate = function() {
...
fmarcasedit.Form_CustomValidate = 
...
fmarcasedit.ValidateRequired = true;
<?php } else { ?>
fmarcasedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmarcasedit.Lists["x_id_idioma"] = {"LinkField":"x_id_idioma","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $marcas_edit->ShowPageHeader(); ?>
<?php
$marcas_edit->ShowMessage();
?>
<form name="fmarcasedit" id="fmarcasedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="marcas">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_marcasedit" class="table table-bordered table-striped">
...
<?php if (@$_POST["fa_x_logo_url"] == "0") { ?>
<input type="hidden" name="fa_x_logo_url" id= "fa_x_logo_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_logo_url" id= "fa_x_logo_url" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_tienda1_url"] == "0") { ?>
<input type="hidden" name="fa_x_tienda1_url" id= "fa_x_tienda1_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_tienda1_url" id= "fa_x_tienda1_url" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_tienda2_url"] == "0") { ?>
<input type="hidden" name="fa_x_tienda2_url" id= "fa_x_tienda2_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_tienda2_url" id= "fa_x_tienda2_url" value="1">
<?php } ?>
...
<?php if (@$_POST["fa_x_tienda3_url"] == "0") { ?>
<input type="hidden" name="fa_x_tienda3_url" id= "fa_x_tienda3_url" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_tienda3_url" id= "fa_x_tienda3_url" value="1">
<?php } ?>
...
<script type="text/javascript">
ew_CreateEditor("fmarcasedit", "x_descripcion1", 35, 4, <?php echo ($marcas->descripcion1->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateEditor("fmarcasedit", "x_descripcion2", 35, 4, <?php echo ($marcas->descripcion2->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
ew_CreateEditor("fmarcasedit", "x_descripcion3", 35, 4, <?php echo ($marcas->descripcion3->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<input type="hidden" data-field="x_id_marca" name="x_id_marca" id="x_id_marca" value="<?php echo ew_HtmlEncode($marcas->id_marca->CurrentValue) ?>">
