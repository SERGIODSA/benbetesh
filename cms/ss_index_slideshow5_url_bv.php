...
$ss_index_slideshow5_url_blobview = NULL; // Initialize page object first

class css_index_slideshow5_url_blobview extends css_index {
	...
	// Page object name
	var $PageObjName = 'ss_index_slideshow5_url_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('ss_index', 'x_slideshow5_url');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('slideshow5_url');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('slideshow5_url')));
				if (trim(strval($this->Recordset->fields('slideshow5_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('slideshow5_url') . "\"");
				}
...
// Create page object
if (!isset($ss_index_slideshow5_url_blobview)) $ss_index_slideshow5_url_blobview = new css_index_slideshow5_url_blobview();

// Page init
$ss_index_slideshow5_url_blobview->Page_Init();

// Page main
$ss_index_slideshow5_url_blobview->Page_Main();
?>
<?php
$ss_index_slideshow5_url_blobview->Page_Terminate();
?>
