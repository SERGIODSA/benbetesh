...
$nosotros_imagen1_url_blobview = NULL; // Initialize page object first

class cnosotros_imagen1_url_blobview extends cnosotros {
	...
	// Page object name
	var $PageObjName = 'nosotros_imagen1_url_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('nosotros', 'x_imagen1_url');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('imagen1_url');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('imagen1_url')));
				if (trim(strval($this->Recordset->fields('imagen1_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('imagen1_url') . "\"");
				}
...
// Create page object
if (!isset($nosotros_imagen1_url_blobview)) $nosotros_imagen1_url_blobview = new cnosotros_imagen1_url_blobview();

// Page init
$nosotros_imagen1_url_blobview->Page_Init();

// Page main
$nosotros_imagen1_url_blobview->Page_Main();
?>
<?php
$nosotros_imagen1_url_blobview->Page_Terminate();
?>
