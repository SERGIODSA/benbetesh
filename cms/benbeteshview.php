...
$benbetesh_view = NULL; // Initialize page object first

class cbenbetesh_view extends cbenbetesh {
	...
	// Page object name
	var $PageObjName = 'benbetesh_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["id_tienda"] <> "") {
			$this->RecKey["id_tienda"] = $_GET["id_tienda"];
			$KeyUrl .= "&amp;id_tienda=" . urlencode($this->RecKey["id_tienda"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_tienda"] <> "") {
				...
				$this->RecKey["id_tienda"] = $this->id_tienda->QueryStringValue;
			} else {
				$sReturnUrl = "benbeteshlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "benbeteshlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "benbeteshlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($benbetesh_view)) $benbetesh_view = new cbenbetesh_view();

// Page init
$benbetesh_view->Page_Init();

// Page main
$benbetesh_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$benbetesh_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var benbetesh_view = new ew_Page("benbetesh_view");
benbetesh_view.PageID = "view"; // Page ID
var EW_PAGE_ID = benbetesh_view.PageID; // For backward compatibility

// Form object
var fbenbeteshview = new ew_Form("fbenbeteshview");

// Form_CustomValidate event
fbenbeteshview.Form_CustomValidate = 
...
fbenbeteshview.ValidateRequired = true;
<?php } else { ?>
fbenbeteshview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fbenbeteshview.Lists["x_id_idioma"] = {"LinkField":"x_id_idioma","Ajax":null,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $benbetesh_view->ExportOptions->Render("body") ?>
<?php if (!$benbetesh_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($benbetesh_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $benbetesh_view->ShowPageHeader(); ?>
<?php
$benbetesh_view->ShowMessage();
?>
<form name="fbenbeteshview" id="fbenbeteshview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="benbetesh">
<table class="ewGrid"><tr><td>
<table id="tbl_benbeteshview" class="table table-bordered table-striped">
<?php if ($benbetesh->id_idioma->Visible) { // id_idioma ?>
	<tr id="r_id_idioma">
		<td><span id="elh_benbetesh_id_idioma"><?php echo $benbetesh->id_idioma->FldCaption() ?></span></td>
...
<?php echo $benbetesh->id_idioma->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($benbetesh->imagen_url->Visible) { // imagen_url ?>
	<tr id="r_imagen_url">
		<td><span id="elh_benbetesh_imagen_url"><?php echo $benbetesh->imagen_url->FldCaption() ?></span></td>
		<td<?php echo $benbetesh->imagen_url->CellAttributes() ?>>
<span id="el_benbetesh_imagen_url" class="control-group">
...
<?php echo $benbetesh->imagen_url->ViewValue ?>
...
<?php if ($benbetesh->titulo->Visible) { // titulo ?>
	<tr id="r_titulo">
		<td><span id="elh_benbetesh_titulo"><?php echo $benbetesh->titulo->FldCaption() ?></span></td>
...
<?php echo $benbetesh->titulo->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($benbetesh->horario->Visible) { // horario ?>
	<tr id="r_horario">
		<td><span id="elh_benbetesh_horario"><?php echo $benbetesh->horario->FldCaption() ?></span></td>
...
<?php echo $benbetesh->horario->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($benbetesh->telefono->Visible) { // telefono ?>
	<tr id="r_telefono">
		<td><span id="elh_benbetesh_telefono"><?php echo $benbetesh->telefono->FldCaption() ?></span></td>
...
<?php echo $benbetesh->telefono->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $benbetesh->dias->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
