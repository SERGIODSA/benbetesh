...
class cnoticias extends cTable {
	...
	function __construct() {
		...
		// id_noticia
		$this->id_noticia = new cField('noticias', 'noticias', 'x_id_noticia', 'id_noticia', '`id_noticia`', '`id_noticia`', 18, -1, FALSE, '`id_noticia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_noticia->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_noticia'] = &$this->id_noticia;

		// id_idioma
		$this->id_idioma = new cField('noticias', 'noticias', 'x_id_idioma', 'id_idioma', '`id_idioma`', '`id_idioma`', 18, -1, FALSE, '`id_idioma`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_idioma->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_idioma'] = &$this->id_idioma;

		// fecha
		$this->fecha = new cField('noticias', 'noticias', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 133, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// titulo
		$this->titulo = new cField('noticias', 'noticias', 'x_titulo', 'titulo', '`titulo`', '`titulo`', 200, -1, FALSE, '`titulo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['titulo'] = &$this->titulo;

		// amigable
		$this->amigable = new cField('noticias', 'noticias', 'x_amigable', 'amigable', '`amigable`', '`amigable`', 200, -1, FALSE, '`amigable`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['amigable'] = &$this->amigable;

		// descripcion
		$this->descripcion = new cField('noticias', 'noticias', 'x_descripcion', 'descripcion', '`descripcion`', '`descripcion`', 201, -1, FALSE, '`descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['descripcion'] = &$this->descripcion;

		// imagen_url
		$this->imagen_url = new cField('noticias', 'noticias', 'x_imagen_url', 'imagen_url', '`imagen_url`', '`imagen_url`', 201, -1, TRUE, '`imagen_url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('id_noticia', $rs))
				ew_AddFilter($where, ew_QuotedName('id_noticia') . '=' . ew_QuotedValue($rs['id_noticia'], $this->id_noticia->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->id_noticia->Exportable) $Doc->ExportCaption($this->id_noticia);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->id_noticia->Exportable) $Doc->ExportField($this->id_noticia);
