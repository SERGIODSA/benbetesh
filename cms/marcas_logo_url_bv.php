...
$marcas_logo_url_blobview = NULL; // Initialize page object first

class cmarcas_logo_url_blobview extends cmarcas {
	...
	// Page object name
	var $PageObjName = 'marcas_logo_url_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('marcas', 'x_logo_url');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('logo_url');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('logo_url')));
				if (trim(strval($this->Recordset->fields('logo_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('logo_url') . "\"");
				}
...
// Create page object
if (!isset($marcas_logo_url_blobview)) $marcas_logo_url_blobview = new cmarcas_logo_url_blobview();

// Page init
$marcas_logo_url_blobview->Page_Init();

// Page main
$marcas_logo_url_blobview->Page_Main();
?>
<?php
$marcas_logo_url_blobview->Page_Terminate();
?>
