...
$nosotros_imagen2_url_blobview = NULL; // Initialize page object first

class cnosotros_imagen2_url_blobview extends cnosotros {
	...
	// Page object name
	var $PageObjName = 'nosotros_imagen2_url_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('nosotros', 'x_imagen2_url');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('imagen2_url');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('imagen2_url')));
				if (trim(strval($this->Recordset->fields('imagen2_url'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('imagen2_url') . "\"");
				}
...
// Create page object
if (!isset($nosotros_imagen2_url_blobview)) $nosotros_imagen2_url_blobview = new cnosotros_imagen2_url_blobview();

// Page init
$nosotros_imagen2_url_blobview->Page_Init();

// Page main
$nosotros_imagen2_url_blobview->Page_Main();
?>
<?php
$nosotros_imagen2_url_blobview->Page_Terminate();
?>
