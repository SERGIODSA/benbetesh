...
$ss_nosotros_add = NULL; // Initialize page object first

class css_nosotros_add extends css_nosotros {
	...
	// Page object name
	var $PageObjName = 'ss_nosotros_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_ssnosotros"] != "") {
				...
				$this->setKey("id_ssnosotros", $this->id_ssnosotros->CurrentValue); // Set up key
			} else {
				$this->setKey("id_ssnosotros", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "ss_nosotrosview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->id_ssnosotros->setDbValue($conn->Insert_ID());
			$rsnew['id_ssnosotros'] = $this->id_ssnosotros->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($ss_nosotros_add)) $ss_nosotros_add = new css_nosotros_add();

// Page init
$ss_nosotros_add->Page_Init();

// Page main
$ss_nosotros_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$ss_nosotros_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var ss_nosotros_add = new ew_Page("ss_nosotros_add");
ss_nosotros_add.PageID = "add"; // Page ID
var EW_PAGE_ID = ss_nosotros_add.PageID; // For backward compatibility

// Form object
var fss_nosotrosadd = new ew_Form("fss_nosotrosadd");

// Validate form
fss_nosotrosadd.Validate = function() {
...
fss_nosotrosadd.Form_CustomValidate = 
...
fss_nosotrosadd.ValidateRequired = true;
<?php } else { ?>
fss_nosotrosadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fss_nosotrosadd.Lists["x_id_idioma"] = {"LinkField":"x_id_idioma","Ajax":null,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $ss_nosotros_add->ShowPageHeader(); ?>
<?php
$ss_nosotros_add->ShowMessage();
?>
<form name="fss_nosotrosadd" id="fss_nosotrosadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="ss_nosotros">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_ss_nosotrosadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
ew_CreateEditor("fss_nosotrosadd", "x_descripcion", 35, 4, <?php echo ($ss_nosotros->descripcion->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<script type="text/javascript">
fss_nosotrosadd.Lists["x_id_idioma"].Options = <?php echo (is_array($ss_nosotros->id_idioma->EditValue)) ? ew_ArrayToJson($ss_nosotros->id_idioma->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fss_nosotrosadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$ss_nosotros_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$ss_nosotros_add->Page_Terminate();
?>
